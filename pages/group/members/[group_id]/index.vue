<template>
    <div class="panel" v-loading="isLoading.isLoadingType('getMemberMembers')">
        ...
        <div
            v-if="isLoading.store.permissions?.includes('Create Members')"
            @click="useMember.modal.create = true"
            class="mb-4 grid h-10 w-full cursor-pointer place-content-center rounded-md border border-white-dark/20 dark:border-[#191e3a]"
        >
            <icon-plus class="h-6 w-6" />
        </div>
        <div v-if="isLoading.store.permissions?.includes('View Members')" class="mb-5">
                        ...
                        <template v-for="(data, index) in useMember.store.member" :key="data.id">
                            <tr>
                                ...
                                <td class="flex items-center gap-2">
                                    <img v-if="data.user?.image" class="h-6 w-6 rounded-full" :src="data.user.image" alt="" />
                                    <img
                                        v-else
                                ...
                                <td class="text-center">
                                    <ul class="flex items-center justify-center gap-3">
                                        <li v-if="isLoading.store.permissions?.includes('Edit Members')" @click="handleTahrirlash(data)">
                                            <client-only>
                                        ...
                                        <li v-if="isLoading.store.permissions?.includes('Delete Members')" @click="handleDelete(data.id)">
                                            <client-only>
        ...
        <div class="full_flex w-full border py-10" v-if="!isLoading.isLoadingType('getMemberMembers') && !useMember.store.member.length">Ma'lumotlar yo'q</div>
                            ...
                            <DialogPanel class="panel w-full max-w-lg rounded-lg border-0 p-0 text-black dark:text-white-dark">
                                ...
                                <form @submit.prevent="useMember.createMember" class="p-5">
                                    <div v-if="!useMember.modal.edit" class="custom-file-container" data-upload-id="myFirstImage">
                                        <p>Azo nomi</p>
                                        <multiselect
                                            v-model="useMember.store.user_id"
                                            :options="useUser.store.users"
                                            @input="searchUsers"
                                    ...
                                    <div class="flex items-center gap-2">
                                        <input id="group_access" v-model="useMember.create.is_capitan" type="checkbox" class="h-5 w-5" />
                                        <label for="group_access">Guruh sardori</label>
                                    </div>

                                    <div class="mt-8 flex items-center justify-end">
                                        <button type="button" @click="useMember.modal.create = false" class="btn btn-outline-danger">Bekor qilish</button>
                            ...
                            <DialogPanel class="panel w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black dark:text-white-dark">
                                ...
                                <div class="p-5 text-center">
                                    ...
                                    <div class="mt-8 flex items-center justify-center">
                                        <button type="button" class="btn btn-outline-danger" @click="useMember.modal.delete = false">Bekor qilish</button>
                                        <button type="button" class="btn btn-primary ltr:ml-4 rtl:mr-4" @click="useMember.deleteMember">O'chirish</button>
                                    </div>
                                </div>
                            </DialogPanel>
                        </TransitionChild>
...
<script setup>
    ...
    if (!isLoading.store.permissions?.includes('View Members')) {
        // router.push("/");
    }
