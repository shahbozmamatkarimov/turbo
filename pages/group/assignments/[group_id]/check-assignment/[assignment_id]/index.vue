    ...
    <div class="panel" v-loading="isLoading.isLoadingType('getCheck')">
        <div class="mb-5 flex items-center justify-between">
            ...
            <div class="flex items-center justify-center" @click="useCheck.modal.is_new = true">
                <button v-if="!useCheck.store.check?.is_check" type="button" class="btn btn-info">Yangilash</button>
            </div>
        </div>
        <div class="mb-5">
                        ...
                        <template v-for="(data, index) in useCheck.store.check.data" :key="data.id">
                            <tr>
                                ...
                                <td class="flex items-center gap-3">
                                    {{ data.points }}
                                    <client-only v-if="!data.is_checked">
        ...
        <div class="full_flex w-full border py-10" v-if="!isLoading.isLoadingType('getCheck') && !useCheck.store.check?.data?.length">Ma'lumotlar yo'q</div>
                    ...
                    >
                        <DialogPanel class="panel w-full max-w-lg overflow-hidden rounded-lg border-0 p-0 text-black dark:text-white-dark">
                            ...
                            <div class="bg-[#fbfbfb] py-3 text-lg font-medium dark:bg-[#121c2c] ltr:pl-5 ltr:pr-[50px] rtl:pl-[50px] rtl:pr-5">
                                Topshiriq baholarini yangilash
                            </div>
                            <div class="p-5 text-center">
                                ...
                                <div class="mx-auto mt-5 sm:w-3/4">Haqiqatdan ham yangilamoqchimisiz?</div>

                                <div class="mt-8 flex items-center justify-center">
                                    <button type="button" class="btn btn-outline-danger" @click="useCheck.modal.is_new = false">Bekor qilish</button>
                                    <button type="button" class="btn btn-primary ltr:ml-4 rtl:mr-4" @click="useCheck.updateCheckList">Yangilash</button>
                                </div>
                            </div>
                        </DialogPanel>
                    </TransitionChild>
