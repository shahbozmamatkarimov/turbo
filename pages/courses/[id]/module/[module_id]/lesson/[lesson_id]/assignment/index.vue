<template>
    <div v-loading="isLoading.isLoadingType('getAssignments')" class="panel">
        ...
        <div class="mb-5">
            <div class="table-responsive">
                <table>
                    ...
                    <tbody>
                        <template v-for="data in useAssignments.store.assignments" :key="data.id">
                            <tr>
                                ...
                                <td @click="nextAttachment(data.id)" class="cursor-pointer">{{ data.lesson?.title }}</td>
                                <td class="text-center">
                                    <ul class="flex items-center justify-center gap-3">
                                        <li v-if="isLoading.store.permissions?.includes('Edit Assignment')" @click="handleTahrirlash(data)">
                                            <client-only>
                                        ...
                                        <li v-if="isLoading.store.permissions?.includes('Delete Assignment')" @click="handleDelete(data.id)">
                                            <client-only>
                ...
                <div class="full_flex w-full border py-10" v-if="!isLoading.isLoadingType('getAssignments') && !useAssignments.store.assignments.length">
                    Ma'lumotlar yo'q
                </div>
            </div>
        </div>
    ...
    <!-- Modal -->
    <client-only>
        <TransitionRoot appear :show="useAssignments.modal.create" as="template">
            <Dialog as="div" @close="useAssignments.modal.create = false" class="relative z-[51]">
                <TransitionChild
                    as="template"
...
<script setup>
    ...
    import { useAssignmentsStore, useLoadingStore } from '@/stores';
    useHead({ title: 'Kurslar' });
    const useAssignments = useAssignmentsStore();
    const isLoading = useLoadingStore();
    isLoading.store.page_name = 'assignments';
    ...
    function nextAttachment(id) {
        router.push(`/attachment/${id}`);
    }

    function handleAssignmentName() {
        try {
            const nameParts = useAssignments.create.name.split(' ');
            useAssignments.create.suffix = nameParts.map((part) => part[0].toUpperCase()).join('');
        } catch (_) {
            useAssignments.create.suffix = '';
        }
    }
    ...
    function handleFileUpload(e) {
        useAssignments.create.image = e.target.files[0];
    }
    ...
    watch(
        () => useAssignments.modal.create,
        async () => {
            if (!useAssignments.modal.create) {
                useAssignments.modal.edit = false;
                useAssignments.clearData();
            }
        }
    );
</script>
